...
{
	public class EraserToolHandle : ITilemapToolHandle
	{
		...
		public EraserToolHandle(Action<IEnumerable<Vector3Int>> select, Action<IEnumerable<Vector3Int>> draw, Action<IEnumerable<Vector3Int>> erase)
		{
			Select = select;
			Draw = draw;
			Erase = erase;
		...
		{
			Erase?.Invoke(new Vector3Int[1] { tile });
		}
